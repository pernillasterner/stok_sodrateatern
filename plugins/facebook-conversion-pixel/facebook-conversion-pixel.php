...
if ( !$upgraded && $has_legacy_save ) {
	...
	*/
	...
	function fca_pc_maybe_add_pixel() {
		...
		$roles_check = 0 === count( array_intersect( array_map( 'strtolower', $roles ), array_map( 'strtolower', $exclude ) ) );
		
		if ( $roles_check ) {
			
			//HOOK IN OTHER INTEGRATIONS/FEATURES
			do_action( 'fca_pc_after_role_check_head', $options );
			...
			if ( is_search() && !function_exists('fca_pc_woocommerce_events') && !function_exists('fca_pc_edd_events') ) {
